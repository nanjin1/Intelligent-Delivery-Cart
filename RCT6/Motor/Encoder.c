...
void Encoder_init(void){
	...
	wheel_2.GPIO_PORT_2 = GPIO_PIN_7;
	...
	//右前
	wheel_3.TIM = htim2;
	wheel_3.GPIO_2 = GPIOB;        //通道2--B相
	wheel_3.GPIO_PORT_2 = GPIO_PIN_3;
	...
	//右后
	wheel_4.TIM = htim5;
	wheel_4.GPIO_2 = GPIOA;        //通道2--B相
	wheel_4.GPIO_PORT_2 = GPIO_PIN_1;
...
void encoder_clear(void){
	...
	motor_all.F_Distance = 0;
	motor_all.R_Distance = 0;
	HAL_TIM_IC_Start_IT(&(wheel_1.TIM), wheel_1.Channel);	  //启动输入捕获
	HAL_TIM_IC_Start_IT(&(wheel_2.TIM), wheel_2.Channel);	  //启动输入捕获
	HAL_TIM_IC_Start_IT(&(wheel_3.TIM), wheel_3.Channel);	  //启动输入捕获
	HAL_TIM_IC_Start_IT(&(wheel_4.TIM), wheel_4.Channel);	  //启动输入捕获

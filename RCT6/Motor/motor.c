...
struct P_pid_obj F_Distance_pid_obj = {0,0,0,0,0,0};
struct P_pid_obj R_Distance_pid_obj = {0,0,0,0,0,0};

struct PID_param Distance_pid_param;
//初始化轮子以及PID参数
...
void pid_init(void){
	...
	line_pid_param.outputMax = 2000;
	
	gyroG_pid_param.kp=3.0f;
	gyroG_pid_param.ki=0.02;
	gyroG_pid_param.kd=0;
	gyroG_pid_param.differential_filterK =1;
	gyroG_pid_param.outputMax=500;
	
	gyroT_pid_param.kp = 2;
	gyroT_pid_param.ki = 0.05f;
	gyroT_pid_param.kd = 0;
	gyroT_pid_param.outputMax = 300;
	gyroT_pid_param.differential_filterK = 1;
	
	Distance_pid_param.kp = 6.0f;
	Distance_pid_param.ki = 0.0;//0.1
	Distance_pid_param.kd = 0.0;//0
	Distance_pid_param.outputMax = 1500;
	Distance_pid_param.outputMin = -1500;
	Distance_pid_param.differential_filterK = 1;
	motor_pid_clear();
}
...
void motor_set_pwm(uint8_t motor, int32_t pid_out){
	...
	if (pid_out >= 0)
	{
		...
		switch (motor)
		{
			case 1: TIM1->CCR3 = 0; TIM1->CCR4 = ccr; break;
			case 2: TIM8->CCR4 = 0; TIM8->CCR3 = ccr; break;
			case 3: TIM1->CCR1 = 0; TIM1->CCR2 = ccr; break;
	...
	else if (pid_out < 0)
	{
		...
		switch (motor)
		{
			...
			case 2: TIM8->CCR3 = 0; TIM8->CCR4 = ccr; break;
			case 3: TIM1->CCR2 = 0; TIM1->CCR1 = ccr; break;
